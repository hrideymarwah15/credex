...
import { ArrowRight, CheckCircle2, TrendingDown, AlertTriangle, Sparkles } from "lucide-react";
    ...
    <div className="space-y-10">
        ...
        {isOptimal ? (
          <div className="relative space-y-3">
            ...
            <p className="text-zinc-600 dark:text-zinc-400 max-w-lg text-lg leading-relaxed">
              We checked every tool against current pricing and your team size. You&apos;re
              not overpaying. Come back when your stack changes.
            </p>
          </div>
        ) : (
          <div className="relative space-y-4">
            ...
            <p className="text-lg sm:text-xl text-zinc-700 dark:text-zinc-300 leading-relaxed">
              That&apos;s <strong className="text-zinc-900 dark:text-white">{formatUsd(totalAnnualSavings)}/year</strong> back in your
              runway.
            </p>
            <p className="text-sm text-zinc-500 dark:text-zinc-500 font-mono">
              {formatUsd(totalCurrentMonthly)}/mo &rarr; {formatUsd(totalRecommendedMonthly)}/mo
            </p>
          </div>
        )}
      </section>
      ...
      <section aria-label="Per-tool breakdown" className="space-y-4">
        ...
        {findings.map((f, i) => (
          <div
            key={i}
            className={`rounded-xl border p-5 sm:p-6 ${severityStyle[f.severity]}`}
          >
          ...
          <div className="relative flex items-start gap-4">
            ...
            <div className="space-y-3">
              ...
              <p className="text-sm text-indigo-900/80 dark:text-indigo-200/80 leading-relaxed max-w-lg">
                At your spend level, custom contracts and committed-use discounts
                typically save another 15-30% on top of the cuts above. Worth a 20-min
                call.
              </p>
              <a
              ...
              >
                Book a consultation <ArrowRight className="w-3.5 h-3.5" aria-hidden="true" />
              </a>
            </div>
          </div>
        </section>
